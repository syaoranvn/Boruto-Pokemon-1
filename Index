...
try {
  ...
  console.log(`[Boruto] Đã load ${apiKeys.length} API keys từ gemini-keys.json`);
} catch (error) {
  console.error("[Boruto] Lỗi load gemini-keys.json:", error.message);
  // Fallback: thử load từ env variable
  if (process.env.GOOGLE_API_KEY) {
    apiKeys = process.env.GOOGLE_API_KEY.split(',').map(k => k.trim());
    console.log(`[Boruto] Fallback: Đã load ${apiKeys.length} API keys từ env`);
  } else {
